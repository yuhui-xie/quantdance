...
| `slope_days` | `60` | 归一化收盘价线性回归窗口（斜率趋势的交易日数，SLOPE_N） |
...
经验上 `slope_days` 在中长窗口（约 30–60）表现较好；过短易追噪音、过长被端点主导。
...
`rotate_threshold=1.0`（关闭惰性），`execution_timing=next_day_open`（决策次日开盘成交）：
...
配方在 2020-2026 子池上约 **17.9 倍**（100000 → ~179 万，即累计 +1690%；年化约 54%、
最大回撤约 25%、Sharpe≈1.62、约 141 次换手）。三条绝对过滤缺一不可——放宽波动率或近期
涨幅上限会把总收益压到 ~10 倍区间、回撤升高。成交时点对结果有明显影响：同参数下
