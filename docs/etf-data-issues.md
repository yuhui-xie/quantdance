...
> ⚠️ 上述抽样结论**不足以排除**残留下限——159922 就不在核心池里、也没被 400 只抽样命中。

## 后续补充（2026-09）：除权日非交易日导致记录被丢弃

ETF **159922** 在 2024-12-02 存在 **-60.6%** 虚假断层（5.951 → 2.343），且该日之前的历史
价格高估 **2.5 倍**。根因不是本页此前修过的 `category==11` 缺失，而是**除权日落在非交易日**：
159922 的份额折算记录日期是周日 **2024-12-01**，按日期 join 时被静默丢弃。

同一缺陷也会命中股票：除权日遇上**停牌**同样没有 bar。抽样 300 只股票，窗口内除权记录
有 **1.5%** 属于此类。详见 [data-adjustment.md](data-adjustment.md) 的
「除权日非交易日导致记录被丢弃（已修复，v5→v6）」。

**排查方法上的教训**：本页原来用「隔夜 |跳变| > 30%」扫描，只能抓到 159922 这种 60% 的
极端case；分红额小的记录被丢弃后只留下 1~3% 的残留缺口，**混在正常波动里根本看不出来**。
更可靠的做法是**逐条核对除权记录的日期是否落在该标的的真实 bar 日历上**（而不是只看价格跳变）。
